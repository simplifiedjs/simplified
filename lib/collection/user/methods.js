...
module.exports = {
    async insert(args) {
        let {login, email, pass, group, isSilent} = args;
        ...
        let [err2, user] = await Collection.User.Model.getRow(false, {ID});
        ...
        if (!isSilent) {
            await Collection.User.trigger( 'insert', user, args );
        }
    ...
    async update(args) {
        let {ID, login, email, pass, group} = args;
        if (!ID) {
        ...
        await Collection.User.trigger( 'update', user, args );
    ...
    async query({group, page, perPage, search}) {
        perPage = perPage || 50;
