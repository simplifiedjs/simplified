...
class MySQL {
    constructor(name, schema = {}, options = {}, actions = {}) {
    ...
    async insert(columns) {
        ...
        let [err, results] = await this.db.query(sql, format);
    ...
    update(columns = {}, where = {}) {
        ...
        return this.db.query(sql, format);
    }
    ...
    get(condition = {}) {
        let table = this.getName(),
            columns = formatColumns(condition.columns, table),
        ...
        return this.db.query(sql, format).then(this.__handleGet);
    }

    async __handleGet([err, results]) {
        if (results && results.length) {
            for (let i = 0; i < results.length; i++) {
                results[i] = await filterColumns(results[i], this.__schema);
            }
        }
